...
class Trainer():
    def __init__(
        ...
        print("Trainer initialized without initializing ROS node.")
    ...
    def get_data(self, idxs):
        ...
        for idx in idxs:
            ...
            if self.dataset_format in ["realsense", "realsense_franka"]:
                goal_reached = sample.get("goal", False)  # 기본값 False
                start_command = sample.get("start", False)  # 기본값 False
                # print("trainer")
                # print(goal_reached)
                # print(start_command)
        ...
        return frames_data, goal_reached, start_command

    def trainer_rotation_angle_between(self,R1, R2):
        """
    ...
    def trainer_is_pose_same(self, pose1, pose2, tolerance=0.17):
        # pose1, pose2: 4x4 변환 행렬
        position_diff = np.linalg.norm(pose1[:3, 3] - pose2[:3, 3])
        R1 = pose1[:3, :3]
        R2 = pose2[:3, :3]
        rotation_diff = self.trainer_rotation_angle_between(R1, R2)
        
        # 위치와 회전 차이가 tolerance 이하이면 동일한 pose로 간주
        # if(position_diff < tolerance and rotation_diff < tolerance):
        #     print("trainer_Position difference: ", position_diff)
        #     print("trainer_Rotation difference(radian): ", rotation_diff)
        return position_diff < tolerance and rotation_diff < tolerance
    
    def find_matching_index(self,existing_T_WC_batch_np, new_T_WC_batch_np, tolerance=0.1):
        
        # if len(new_T_WC_batch_np.shape) == 2:  # 4 x 4 형태인 경우
        #     new_T_WC_batch_np = np.expand_dims(new_T_WC_batch_np, axis=0)  # (1 x 4 x 4)

        # length = existing_T_WC_batch_np.shape[0]
        # print("Length of existing_T_WC_batch_np:", length)
        
        # differences = np.linalg.norm(existing_T_WC_batch_np - new_T_WC_batch_np, axis=(1, 2))

        # matching_indices = np.where(differences < tolerance)[0]
        
        matching_indices = []
        for i in range(existing_T_WC_batch_np.shape[0]):
                if self.trainer_is_pose_same(new_T_WC_batch_np, existing_T_WC_batch_np[i]):
                    matching_indices.append(i)
        if len(matching_indices) > 0:
            print(f"Matching index found: {matching_indices}")
            return matching_indices  # 첫 번째 일치하는 인덱스 반환
        else:
            return -1  # 일치하는 인덱스 없음
        
        # if len(matching_indices) > 0:
        #     print(f"Matching index found: {matching_indices}")
        #     print(f"first matching index: {matching_indices[0]}")
        #     return matching_indices[0]  # 첫 번째 일치하는 인덱스 반환
        # else:
        #     return -1  # 일치하는 인덱스 없음
    
    def exchange_frame(self, framedata):
        ...
        if isinstance(index, list) and len(index) > 0:
            ...
            # if len(index) > 1:
            #     self.exchange_data_mcy(framedata, index[0])
            #     self.exchange_data_mcy(framedata, index[1])
            # else:
                # self.exchange_data_mcy(framedata, index[0])
                self.exchange_data_mcy(framedata, index[i])
        elif index == -1:
            print("No matching index found.")
        
    def exchange_data_mcy(self, data, index=None):

        self.frames.exchange_frame_data(data,index)
        self.steps_since_frame = 0
        # self.last_is_keyframe = False
